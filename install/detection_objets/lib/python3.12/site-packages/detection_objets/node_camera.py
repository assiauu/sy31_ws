...
class CameraNode(Node):
    ...
    def image_callback(self, msg):
       # Vérifie si 2 secondes se sont écoulées prcq sinon trop rapide les couleurs 
        maintenant = self.get_clock().now()
        duree = (maintenant - self.dernier_affichage).nanoseconds / 1e9
        if duree < 1.0:
           return
        self.dernier_affichage = maintenant
        np_arr = np.frombuffer(msg.data, np.uint8) #donc ca ca permet de rendre objet pythonnn
        ...
        #maintenant on convertie bgr en hsv parceque plus precis et plus simple blabla
        image_hsv = cv2.cvtColor(image_bgr, cv2.COLOR_BGR2HSV)
        #definir les plages de couleurs pour le rouge et le bleu seulement fin ce qu'on a besonnn pour projet 
        masque_rouge = cv2.bitwise_or(
          cv2.inRange(image_hsv, (0,   130, 80), (10,  255, 255)),  # rouge côté 0
          cv2.inRange(image_hsv, (170, 130, 100), (180, 255, 255))   
         ) 
        masque_bleu =cv2.inRange(image_hsv,(80,100,100), (130,255,255))
        #on doit mtn afficher quel couleur est apparu dans limage 
